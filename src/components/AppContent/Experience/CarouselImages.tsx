...
const NEBULA_PROJECT = "Nebula";
const MOBILE_BREAKPOINT = "(max-width: 767px)";

export default function CarouselImages({ selectedProjectImages, projectName }: CarouselImagesProps) {
    const [currentImageIndex, setCurrentImageIndex] = useState(0);
    const [shouldShowRotatePrompt, setShouldShowRotatePrompt] = useState(false);

    useEffect(() => {
        const checkOrientation = () => {
            const isMobile = window.matchMedia(MOBILE_BREAKPOINT).matches;
            const isPortrait = window.matchMedia("(orientation: portrait)").matches;
            const isWebScreenshotProject = projectName !== NEBULA_PROJECT;

            setShouldShowRotatePrompt(isMobile && isPortrait && isWebScreenshotProject);
        };
    ...
    if (!selectedProjectImages || selectedProjectImages.length === 0) {
        return null;
    }
